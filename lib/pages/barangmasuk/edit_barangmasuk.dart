...
class EditBarangMasukPage extends StatefulWidget {
  final DataMasuk barangMasuk;

  const EditBarangMasukPage({Key? key, required this.barangMasuk})
      : super(key: key);

  @override
  State<EditBarangMasukPage> createState() => _EditBarangMasukPageState();
}

class _EditBarangMasukPageState extends State<EditBarangMasukPage> {
  ...
  void initState() {
    ...
    // Isi data awal dari barang masuk
    _selectedIdBarang = widget.barangMasuk.idBarang;
    _selectedKodeBarang = widget.barangMasuk.kodeBarang;
    _jumlahController.text = widget.barangMasuk.jumlah?.toString() ?? '';
    _ketController.text = widget.barangMasuk.ket ?? '';
    _tglMasuk = widget.barangMasuk.tglMasuk ?? DateTime.now();
  }
  ...
  Future<void> _pickDate() async {
    final pickedDate = await showDatePicker(
      context: context,
      initialDate: _tglMasuk ?? DateTime.now(),
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      try {
        await BarangMasuksService.updateBarangMasuk(
          widget.barangMasuk.id!,
          {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Barang masuk berhasil diperbarui')),
        );
        Navigator.pop(context, true);
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal memperbarui barang masuk: $e')),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Edit Barang Masuk')),
      body: Padding(
